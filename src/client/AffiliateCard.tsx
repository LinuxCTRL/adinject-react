...
export interface AffiliateCardProps {
	...
	/** Accessible label override (default: "Sponsored product: [Title]") */
	a11yLabel?: string;
	className?: string;
	...
	if (variant === "minimal") {
		return (
			<a
				...
				aria-label={a11yLabel || `Sponsored product: ${product.title}`}
				onClick={handleClick}
			...
			>
				...
				{formattedPrice && (
					<span className="opacity-80">({formattedPrice})</span>
				)}
